...
#Gionee <Amigo_ENV> lingfen modify it for no subdir  begin
    #my_module_path := $(my_module_path)/$(LOCAL_MODULE)
    my_module_path := $(my_module_path)
#Gionee <Amigo_ENV> lingfen modify it for no subdir end
  endif
  endif
...
#Gionee <Amigo_ENV> lingfen modify it for sonar begin
ifeq ($(GN_BUILD_SONAR_ANT),yes)
ifneq ($(LOCAL_PACKAGE_NAME),)
$(warning lingfen debug -> $(LOCAL_PACKAGE_NAME))
gn_libs := $(full_java_libs) $(full_shared_java_libs) $(full_java_lib_deps)
$(shell pushd $(GN_BUILD_ROOT_ENV_DIR)/ant/ >/dev/null; ./main.sh $(LOCAL_PACKAGE_NAME) "$(gn_libs)" $(call intermediates-dir-for, APPS, $(LOCAL_PACKAGE_NAME),,COMMON); popd >/dev/null)
endif
endif
#Gionee <Amigo_ENV> lingfen modify it for sonar end
